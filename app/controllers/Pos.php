...
class Pos extends MY_Controller
{
                ...
                if ($suspended_sale = $this->pos_model->getOpenBillByID($sid)) {
                    ...
                    foreach ($inv_items as $item) {
                        ...
                        if ($row->tax_rate) {
                            ...
                            $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'category' => $row->category_id, 'tax_rate' => $tax_rate, 'options' => $options);
                        } else {
                            $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'category' => $row->category_id, 'tax_rate' => FALSE, 'options' => $options);
                        }
                        $c++;
                    }
    ...
    public function getProductDataByCode($code = NULL, $warehouse_id = NULL)
    {
        ...
        if ($row) {
            ...
            if (!$this->Settings->overselling && $row->quantity < 1) {
                echo NULL; die();
            }
            if ($options) {
                $option_quantity = 0;
