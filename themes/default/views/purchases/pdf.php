            ...
            <div class="well well-sm">
                <div class="row bold">
                    ...
                    <div class="col-xs-6 pull-right text-right">
                        <?php $br = $this->sma->save_barcode($inv->reference_no, 'code128', 35, false, $inv->id);?>
                        <img src="<?=base_url()?>assets/uploads/barcode<?=$this->session->userdata('user_id') . $inv->id;?>.png"
                             alt="<?=$inv->reference_no?>"/>
                        <?php $this->sma->qrcode('link', urlencode(site_url('purchases/view/' . $inv->id)), 1, false, $inv->id);?>
                ...
                <table class="table table-bordered table-hover table-striped">
                    ...
                    <?php if ($return_purchase) {
                        ...
                        if ($return_purchase->surcharge != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right;">' . lang("return_surcharge") . ' (' . $default_currency->code . ')</td><td style="text-align:right;">' . $this->sma->formatMoney($return_purchase->surcharge) . '</td></tr>';
                        }
                    }
                    ?>
                    <?php if ($inv->order_discount != 0) {
                        echo '<tr><td colspan="' . $col . '" style="text-align:right;">' . lang("order_discount") . ' (' . $default_currency->code . ')</td><td style="text-align:right;">' . $this->sma->formatMoney($return_purchase ? ($inv->order_discount+$return_purchase->order_discount) : $inv->order_discount) . '</td></tr>';
                    }
                    ?>
