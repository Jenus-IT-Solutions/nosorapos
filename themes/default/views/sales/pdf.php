            ...
            <div class="row padding10">
                ...
                <div class="col-xs-5">
                    <div class="bold">
                        ...
                        <div class="clearfix"></div>
                        <?php $this->sma->qrcode('link', urlencode(site_url('sales/view/' . $inv->id)), 2, FALSE, $inv->id); ?>
                        <img src="<?= base_url() ?>assets/uploads/qrcode<?= $this->session->userdata('user_id') . $inv->id; ?>.png"
                             alt="<?= $inv->reference_no ?>" class="pull-right"/>
                        <?php $br = $this->sma->save_barcode($inv->reference_no, 'code128', 60, FALSE, $inv->id); ?>
                ...
                <table class="table table-bordered table-hover table-striped">
                    ...
                    <?php if ($return_sale) {
                        ...
                        if ($return_sale->surcharge != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right;">' . lang("return_surcharge") . ' (' . $default_currency->code . ')</td><td style="text-align:right;">' . $this->sma->formatMoney($return_sale->surcharge) . '</td></tr>';
                        }
                    }
                    ?>
                    <?php if ($inv->order_discount != 0) {
                        echo '<tr><td colspan="' . $col . '" style="text-align:right;">' . lang('order_discount') . ' (' . $default_currency->code . ')</td><td style="text-align:right;">' . $this->sma->formatMoney($return_sale ? ($inv->order_discount+$return_sale->order_discount) : $inv->order_discount) . '</td></tr>';
                    }
                    ?>
